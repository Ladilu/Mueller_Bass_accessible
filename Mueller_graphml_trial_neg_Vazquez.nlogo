extensions [ nw csv]
globals [color-mode pop %Adopted %Network %Broadcast memoryad att-file] ;; 0 = default, 1 = source, 2 = times heard, 3 = popularity

turtles-own [ adopted? seed? net-adopted? threshold]
links-own [weight]
...
to create-network
  wire_graphml;; create network from graphml
  reset-ticks
  if layout? [layout]
...
to wire_graphml
  clear-all
  nw:load-graphml "Uncorrelated_Vazquez2.graphml" ;;"Assortative_BM_500.graphml"
  repeat 30 [ layout-spring turtles links 0.2 5 1 ]
  ask links [set weight -1.0 + random-float 2.0
        set label weight
        ]
  let degree [count my-links] of turtles
  let sumd sum degree
...
to go
  ...
  set memoryad[]
  set memoryad lput num_adopted memoryad
  output-print num_adopted
...
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
to compute-threshold ;; threshold procedure
  let neighbors-adopted-pos  count link-neighbors with [adopted? AND ([weight] of link-with myself > 0)]
  let neighbors-adopted-neg  count link-neighbors with [adopted? AND ([weight] of link-with myself < 0)]
  set threshold neighbors-adopted-pos - neighbors-adopted-neg ;; + if f-foe = 1, - if f-foe=-1
end
to adopt
 ; ifelse random-float 1.0 < broadcast-influence [
  ...
  if count total-neighbors > 0[
    if not adopted? and  threshold > 0[
    set adopted? true
  ...
  set %Adopted num_adopted ;  ((count turtles with [color = yellow] + count turtles with [color = red] + count turtles with [color = lime] )/ count turtles) * 100
  ;set memoryad lput %Adopted memoryad
...
to-report num_adopted
  report ((count turtles with [color = yellow] + count turtles with [color = red] + count turtles with [color = lime] )/ count turtles) * 100
end
to-report get-att-vals ;turtle proc
  report (list ticks memoryad)
end

to export-att-vals ;turtle proc
   set att-file (word "turtle.csv")
   file-open att-file
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot %Adopted"
"pen-1" 1.0 0 -7500403 true "" "plot %Broadcast"
"pen-2" 1.0 0 -2674135 true "" "plot %Network"
